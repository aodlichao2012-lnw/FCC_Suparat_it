...

        {
            ...
            if (Response.StatusCode == 200)
            {
                try
                {
                    ...
                    DataTable dt = BAACReportDAL.Instance.GetFoodReport(dr["startDate"] + string.Empty, dr["endDate"] + string.Empty);
        ...
        public object Export()
        {
            if (Response.StatusCode == 200)
            {
                try
                {
                    ...
                    ExcelFile workbook = ExcelFile.Load(Server.MapPath("~/Template/FoodReport_Template.xlsx"));
                    ...
                    dt = BAACReportDAL.Instance.GetFoodReport(sdate, edate);

                    wsTemp.Cells["B2"].Value = "ตารางการแลกสิทธิ์หมวดอาหารและเครื่องดื่ม ประจำช่วงวันที่ " + sd.ToString("dd MMMMM yyyy", new CultureInfo("th-TH")) + " - " + ed.ToString("dd MMMMM yyyy", new CultureInfo("th-TH"));
                    ...
                    for (i = 0; i < dt.Rows.Count; i++)
                    {
                        try
                        {
                            ...
                            wsTemp.Cells["K" + (i + 6)].Value = dt.Rows[i]["วันที่นัดหมาย"].ToString().Trim();
                            wsTemp.Cells["K" + (i + 6)].Style.Borders.SetBorders(MultipleBorders.All, SpreadsheetColor.FromName(ColorName.Black), LineStyle.Thin);

                            wsTemp.Cells["L" + (i + 6)].Value = dt.Rows[i]["ServiceType_Name1"].ToString().Trim();
                            ...
                            wsTemp.Cells["R" + (i + 6)].Value = dt.Rows[i]["Delivery_Time"].ToString().Trim();
                    ...
                    string fileName = "ตารางการแลกสิทธิ์หมวดอาหารและเครื่องดื่ม ประจำช่วงวันที่ " + sd.ToString("dd MMMMM yyyy", new CultureInfo("th-TH")) + " - " + ed.ToString("dd MMMMM yyyy", new CultureInfo("th-TH")) + ".xlsx";
