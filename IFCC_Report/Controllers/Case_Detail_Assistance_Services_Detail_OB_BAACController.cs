    ...
    public class Case_Detail_Assistance_Services_Detail_OB_BAACController : Controller
    {
        ...
        public object Export()
        {
            if (Response.StatusCode == 200)
            {
                ...
                {
                    string a = Url.Action().ToString().ToLower();
                    if (a == "/case_detail_assistance_services_detail_ob_baac/export/b1")
                    {
                        DataSet ds = DataHelper.GetRequestData(HttpContext);
                        DataRow dr = ds.Tables[0].Rows[0];
                    ...
                    if (a == "/case_detail_assistance_services_detail_ob_baac/export/b2")
                    {
                        DataSet ds = DataHelper.GetRequestData(HttpContext);
                        DataRow dr = ds.Tables[0].Rows[0];

                        SpreadsheetInfo.SetLicense("ETZX-IT28-33Q6-1HA2");
                        ExcelFile workbook = ExcelFile.Load(Server.MapPath("~/Template/Book2.xlsx"));
                        GemBox.Spreadsheet.ExcelRow er = null;
                        GemBox.Spreadsheet.ExcelWorksheet wsTemp2 = workbook.Worksheets[0];
                        ////Sheet Detail
