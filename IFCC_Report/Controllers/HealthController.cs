...

        {
            ...
            if (Response.StatusCode == 200)
            {
                try
                {
                    ...
                    DataTable dt = BAACReportDAL.Instance.GetHealthReport(dr["startDate"] + string.Empty, dr["endDate"] + string.Empty);
        ...
        public object Export()
        {
            if (Response.StatusCode == 200)
            {
                try
                {
                    ...
                    ExcelFile workbook = ExcelFile.Load(Server.MapPath("~/Template/PackageHealthReport_Template.xlsx"));
                    ...
                    dt = BAACReportDAL.Instance.GetHealthReport(sdate, edate);

                    wsTemp.Cells["B2"].Value = "ตารางการแลกสิทธิ์หมวดแพ็คเกจดูแลสุขภาพ ประจำช่วงวันที่ " + sd.ToString("dd MMMMM yyyy", new CultureInfo("th-TH")) + " - " + ed.ToString("dd MMMMM yyyy", new CultureInfo("th-TH"));
                    ...
                    string fileName = "ตารางการแลกสิทธิ์หมวดแพ็คเกจดูแลสุขภาพ ประจำช่วงวันที่ " + sd.ToString("dd MMMMM yyyy", new CultureInfo("th-TH")) + " - " + ed.ToString("dd MMMMM yyyy", new CultureInfo("th-TH")) + ".xlsx";
