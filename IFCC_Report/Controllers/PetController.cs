...

        {
            ...
            if (Response.StatusCode == 200)
            {
                try
                {
                    ...
                    DataTable dt = BAACReportDAL.Instance.GetPetReport(dr["startDate"] + string.Empty, dr["endDate"] + string.Empty);
        ...
        public object Export()
        {
            if (Response.StatusCode == 200)
            {
                try
                {
                    ...
                    ExcelFile workbook = ExcelFile.Load(Server.MapPath("~/Template/PetReport_Templete.xlsx"));
                    ...
                    dt = BAACReportDAL.Instance.GetPetReport(sdate, edate);

                    wsTemp.Cells["B2"].Value = "ตารางการแลกสิทธิ์หมวดสัตว์เลี้ยง ประจำช่วงวันที่ " + sd.ToString("dd MMMMM yyyy", new CultureInfo("th-TH")) + " - " + ed.ToString("dd MMMMM yyyy", new CultureInfo("th-TH"));
                    ...
                    string fileName = "ตารางการแลกสิทธิ์หมวดสัตว์เลี้ยง ประจำช่วงวันที่ " + sd.ToString("dd MMMMM yyyy", new CultureInfo("th-TH")) + " - " + ed.ToString("dd MMMMM yyyy", new CultureInfo("th-TH")) + ".xlsx";
