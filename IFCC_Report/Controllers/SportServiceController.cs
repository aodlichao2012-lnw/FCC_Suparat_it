...
{
    public class SportServiceController : Controller
    {
        // GET: SportService
...

        {
            ...
            if (Response.StatusCode == 200)
            {
                try
                {
                    ...
                    DataTable dt = BAACReportDAL.Instance.GetSportServiceReport(dr["startDate"] + string.Empty, dr["endDate"] + string.Empty);
        ...
        public object Export()
        {
            if (Response.StatusCode == 200)
            {
                try
                {
                    ...
                    ExcelFile workbook = ExcelFile.Load(Server.MapPath("~/Template/SportServiceReport_Templete.xlsx"));
                    ...
                    dt = BAACReportDAL.Instance.GetSportServiceReport(sdate, edate);

                    wsTemp.Cells["B2"].Value = "ตารางการแลกสิทธิ์หมวดกีฬา ประจำช่วงวันที่ " + sd.ToString("dd MMMMM yyyy", new CultureInfo("th-TH")) + " - " + ed.ToString("dd MMMMM yyyy", new CultureInfo("th-TH"));
                    ...
                    string fileName = "ตารางกระเช้าของขวัญหมวดกีฬา ประจำช่วงวันที่ " + sd.ToString("dd MMMMM yyyy", new CultureInfo("th-TH")) + " - " + ed.ToString("dd MMMMM yyyy", new CultureInfo("th-TH")) + ".xlsx";
